...
procedure AoC_2020_19_full_Ada is
  ...
  begin
    for count in 1 .. n loop
      ...
      if c = '"' then
        ...
        loop
          if (c >= '0') and (c <= '9') then
            j := Character'Pos (c) - Character'Pos ('0');
          else
            j := 0;
          end if;
          k := k + 1;
          while not End_Of_Line (f) loop
            Get (f, c);
            exit when (c < '0') or (c > '9');
            j := j * 10 + Character'Pos (c) - Character'Pos ('0');
          end loop;
          rule (i).sub (k) := j;
  ...
  total : Natural;
  f : File_Type;
  Filename: String := "/Users/user/common/Ada/AoC2020/Day19/example2.txt";
begin
