...
procedure Main is
   ...
   -- detect an infinite loop by storing the lines executed in a set

   function Line_Hash(Line: Natural) return Ada.Containers.Hash_Type is
   ( Ada.Containers.Hash_Type(Line) );

   package Executed_Sets is new Ada.Containers.Hashed_Sets
   (
   ...
   Executed_Lines: Executed_Sets.Set;
   ...
   begin
      ...
      while not (
            ...
            Executed_Lines.Contains(Program_Index)
      ) loop

         Executed_Lines.Insert(Program_Index);
...
begin
   ...
   --
   -- i tried to fiture out a more elegant solution,
